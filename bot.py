...
REGISTER_NAME, REGISTER_SURNAME = range(2)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_message = (
        f"Привет, {db_user.first_name or user.first_name or ''}! 👋\n\n"
...
async def register_surname(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not db_user:
        ...
        db_user.first_name = context.user_data.get('first_name')
        db_user.last_name = context.user_data.get('last_name')
        db.commit()
    db.close()
    await update.message.reply_text("Регистрация завершена! Теперь вы можете пользоваться ботом. Введите /start ещё раз.")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        purchase.scanned_by_admin_id = update.effective_user.id
        purchase.scanned_at = datetime.utcnow()
